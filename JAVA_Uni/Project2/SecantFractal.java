/*
 * PROJECT II: SecantFractal.java
 *
 * This file contains a template for the class SecantFractal. Not all methods
...
public class SecantFractal {
     ...
     */
    public SecantFractal(Polynomial p, Complex origin, double width) {
    ...
    public void printRoots() {
        String str = roots.get(0).toString();
        for (int i = 1; i<= roots.size()-1;i++){
            str = str + roots.get(i).toString();
        }
    ...
    public int findRoot(Complex root) {
        ...
        for(int i=0;i<=roots.size()-1;i++){
            a = roots.get(i);
            if((a.add(root.minus())).abs()<Secant.TOL){
                return i;
            }
        }
        return -1;
    ...
    public Complex pixelToComplex(int i, int j) {
        ...
        return new Complex(origin.getReal()+dz*i,origin.getImag()-dz*j);
        // You need to fill in this function. (Completed)
    }
    ...
    /**
     * Generate the fractal image. See the colourIterations instance variable
     * for a better description of its purpose.
     */
    public void createFractal(boolean colourIterations) {
        Complex pxc; // Creates complex to store value of pixel transformed into complex
        Complex a = new Complex(); // Creates a complex 0
        Complex rt; // Stores roots of the polynomial
        this.colourIterations = colourIterations;

        // Iterates over all pixels in image
        for(int i=0; i< NUMPIXELS; i++){
            for(int j=0; j<NUMPIXELS; j++){
                pxc = pixelToComplex(i, j);

                // Uses secant method on 0 and pxc (each pixel of image transformed into complex)
                iterator.iterate(a,pxc); 

                // If root is found, sets rt to that root and checks if it is in arraylist
                if(iterator.getError()==0){
                    rt = iterator.getRoot();

                    if(findRoot(rt)==-1){

                        // If it is not in arraylist, it adds it and colours the corresponding pixel
                        roots.add(rt);
                        colourPixel(i,j,roots.size()-1,iterator.getNumIterations());
                    }
                        // If root is in array, colours the pixel with the same colour as the root in array
                    else{
                        colourPixel(i,j,findRoot(rt),iterator.getNumIterations());
                    }              
                }
                else{
                }
    ...
    public static void main(String[] args) {  
        ...
        Complex[] coeff = new Complex[] { new Complex(7.0,0.0),  new Complex(), new Complex(), new Complex(3.0), new Complex(1.0,0.0) };
